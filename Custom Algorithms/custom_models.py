    ...
    def forward(self, obs, test=False, with_logprob=True):
        ...
        if with_logprob:
            ...
            logp_pi -= (2 * (np.log(2) - pi_action - F.softplus(-2 * pi_action))).sum(
                axis=1)  # FIXME: this formula is mathematically wrong, no idea why it seems to work
...
def rnn(input_size, rnn_size, rnn_len):
    ...
    gru = nn.GRU(input_size=input_size, hidden_size=hidden_size, num_layers=num_rnn_layers, bias=True, batch_first=True,
